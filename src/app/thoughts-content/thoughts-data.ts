...
A subtler gotcha showed up in the React app's Tailwind v4 integration. Tailwind's @theme block creates new CSS custom properties. Writing --color-background: var(--color-background) looks like a passthrough, but it actually creates a circular reference — the variable points at itself. Per the CSS spec, self-referencing custom properties resolve to the "guaranteed-invalid value," so every Tailwind utility for colors, shadows, and radii silently broke. The fix was referencing the --paul-* prefixed source tokens (--color-background: var(--paul-color-background)). This doesn't affect Angular since we use SCSS variable mapping instead of Tailwind's @theme, but it's worth knowing if you ever bridge design tokens into a Tailwind v4 app.
...
What makes this an enterprise pattern rather than a demo is that nothing here is bespoke. There is no custom prerender script, no hand-maintained route manifest, no separate SEO build step. It is Angular's own render modes, its own Title and Meta services, its own hydration escape hatch — composed so that the pages that need to be fast and indexable are static HTML, and the parts that are pure interaction stay a client app. That is the judgement SSR actually asks of you: not on or off, but which mode each route deserves.`,
  ...
  },
  {
    ...
    content: `Building SSR is one thing; standing it up in production behind a CDN is where the interesting decisions live. This is the log of getting the prerendered Thoughts pages onto a real domain, and the tradeoffs I made along the way.

The first decision was where to run it. The app builds to an Express server that renders on demand and serves the prerendered HTML for the content routes — a long-lived Node process, not a bag of serverless functions. That shape points at a platform that runs the process I already have. Vercel can host Angular SSR, but it wants an adapter and reframes the server as functions; Railway just runs the Node process, hands it the PORT it injects, and gets out of the way. For a server that is genuinely a server, that was the lower-friction choice — and I was already running an API on Railway, so the operational surface stayed small.

I split the deploy into two services from the start: develop → develop.angular.paulsumido.com as staging, and the release branch → angular.paulsumido.com as production. Two branches, two domains, one repo. Staging-first is not ceremony — the first real deploy of a server-rendered app always surfaces something the local build never does, and I would rather find it on a URL nobody has bookmarked. That instinct paid for itself within the hour.

Config-as-code over dashboard clicks. The default npm start in an Angular project is ng serve — the dev server — which is precisely what you do not want a platform to boot in production. Rather than hand-set a start command in a dashboard and hope I remember it for the next service, I pinned it in railway.json: Nixpacks builder, a start command that runs the built server bundle, and a Node engine floor. Both services inherit it. The config file is the source of truth; the dashboard is a view onto it.

Then Angular 21's security model showed its teeth, correctly. The SSR server is secure-by-default: it runs an SSRF host check and rejects any Host header it has not been told to trust, and it ignores X-Forwarded-* headers unless you opt in. Behind a proxy, both fire — the platform terminates TLS and forwards the request, so the app sees a proxied host and forwarded headers it will not trust blindly. The resolution is two settings, not a workaround: allowedHosts in angular.json to authorize the real domains, and trustProxyHeaders: true on the node engine so Angular reconstructs the original URL from the forwarded headers instead of the internal one. I used a wildcard — *.paulsumido.com — because the matcher does an endsWith on *.-prefixed entries, so one line authorizes both the staging subdomain and the production host at any depth. That is a default I want on by default; the only work is telling the framework which hosts you own.

The sharpest infrastructure detail was TLS depth at the edge. develop.angular.paulsumido.com sits two labels below the zone, and Cloudflare's Universal SSL only covers the apex and a single level of wildcard. Proxy that record and the edge presents a certificate that does not match the hostname — a silent handshake failure. So I left the staging record DNS-only and let Railway issue the certificate for the exact hostname; the two-level depth stops mattering the moment you stop proxying it. Production lives one level down, inside Universal SSL's coverage, so there the proxy choice is free. A small distinction with a quiet failure mode if you miss it.

A few deliberate compromises. Canonical and og:url are pinned to the production origin even when the page is served from staging — I do not want staging indexed under its own URL, and pointing canonical at production is the honest signal to a crawler that the real page lives elsewhere. The essay bodies ship in the initial bundle, which nudges it just past the warning budget; I am keeping that number visible rather than raising the threshold, because the correct fix is lazy-loading the content, not hiding the metric. And I wrote a config-guard test that pins allowedHosts and trustProxyHeaders — the failure mode of both is a silent 400 in production, and the cheapest place to catch that is a unit test that goes red the instant someone edits them out.

The payoff is measurable, which is the entire reason to reach for SSR. curl the deployed URL with JavaScript disabled and you get the whole article in the first response: the title, the Open Graph tags a social card reads, the canonical link, and a JSON-LD BlogPosting block — all generated at build time. The desktop shell stays a client app because it should; the content stays static HTML because it should. Two rendering strategies, chosen per route, both proven on a live domain behind a CDN. That is the version of "we use SSR" that survives someone opening the network tab.`,
    relatedApp: 'readme',
  },
];
